...
import {
  SPACE_WAS_DELETED_MESSAGE,
  SpaceWasDeletedMessage,
...
export class SpaceWasDeletedProjection {
  ...
  async handle(message: SpaceWasDeletedMessage) {
    const spaceView = await this.spaceProjection.findById(message._id).exec();

    spaceView.remove();

    const bookingViews = await this.bookingFinder.findSpacePendingBookings(
      BookingSpaceId.fromString(message._id),
    );

    bookingViews.map((bookingView) => {
      this.commandBus.execute<ICommand, Result<null, BookingError>>(
        new DeleteBookingCommand(bookingView._id),
      );
    });
