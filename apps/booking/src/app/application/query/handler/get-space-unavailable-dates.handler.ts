...
export class GetSpaceUnavailableDatesHandler implements IQueryHandler {
    ...
    async execute(query: GetSpaceUnavailableDatesQuery): Promise<Result<Array<string>, BookingSpaceNotFoundError>> {

        const spaceDTO = await this.spaceFinder.find(BookingSpaceId.fromString(query.spaceId))

        if (spaceDTO === null) return new Err(BookingSpaceNotFoundError.withSpaceId(BookingSpaceId.fromString(query.spaceId)))

        return new Ok(await this.bookingFinder.findSpaceUnavailableDates(BookingSpaceId.fromString(query.spaceId), spaceDTO.quantity));
    }
}
