...
export class CompanyWasDeletedMessageProducer implements IEventHandler<CompanyWasDeletedEvent> {
    constructor(
        @Inject('DATASERVICE')
    ...
    async handle(event: CompanyWasDeletedEvent) {
        this.companyProducerClient.emit(COMPANY_WAS_DELETED_MESSAGE, JSON.stringify(event.payload));
    }
}
