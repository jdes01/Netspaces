...
export class CompanyService {
  ...
  async createCompany(
    ...
    return this.commandBus.execute<ICommand, Result<null, CompanyError>>(
      new CreateCompanyCommand(id, name),
    );
  }
  ...
  async updateCompany(
    ...
    return this.commandBus.execute<ICommand, Result<null, CompanyError>>(
      new UpdateCompanyCommand(id, name),
    );
  }

  async getCompanys() {
    return this.queryBus.execute<IQuery, Array<CompanyDTO>>(
      new GetCompanysQuery(),
    );
  }

  async getCompanyById(id: string) {
    return this.queryBus.execute<IQuery, CompanyDTO>(
