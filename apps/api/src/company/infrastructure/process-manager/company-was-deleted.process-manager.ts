import { CommandBus, EventsHandler, ICommand, IEventHandler } from '@nestjs/cqrs';
...
export class CompanyWasDeletedProcessManager implements IEventHandler<CompanyWasDeletedEvent> {
    constructor(
        @Inject(USER_FINDER)
    ...
    async handle(event: CompanyWasDeletedEvent) {
        const companyUsers = await this.userFinder.findByCompanyId(CompanyId.fromString(event.id))

        if (companyUsers.length > 0) {
            companyUsers.map(
                (user) => { this.commandBus.execute<ICommand, Result<null, UserError>>(new DeleteUserCommand(user._id)); }
            )
        }

        const companyWorkspaces = await this.workspaceFinder.findByCompanyId(CompanyId.fromString(event.id))

        if (companyWorkspaces.length > 0) {
            companyWorkspaces.map(
                (workspace) => { this.commandBus.execute<ICommand, Result<null, UserError>>(new DeleteWorkspaceCommand(workspace._id)); }
            )
        }
    }
}
