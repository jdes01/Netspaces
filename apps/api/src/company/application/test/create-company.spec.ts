...
describe('CreateCompanyHandler', () => {
    ...
    it('should creates a new company successfully', async () => {
        ...
        const handler = new CreateCompanyHandler(companyRepository, companyFinder, redisService);
    ...
    it('should return error with existing company', async () => {
        ...
        expect(result).toEqual(new Err(CompanyAlreadyExistsError.withId(CompanyId.fromString(id))))
    });
});
