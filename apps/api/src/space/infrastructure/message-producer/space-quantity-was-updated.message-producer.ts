...
export class SpaceQuantityWasUpdatedMessageProducer implements IEventHandler<SpaceQuantityWasUpdatedEvent> {
    constructor(
        @Inject('DATASERVICE')
    ...
    async handle(event: SpaceQuantityWasUpdatedEvent) {
        this.spaceProducerClient.emit(SPACE_QUANTITY_WAS_UPDATED_MESSAGE, JSON.stringify(event.payload));
    }
}
