...
import {
  WORKSPACE_FINDER,
  WorkspaceFinder,
} from '../../service/workspace-finder.service';
...
export class GetWorkspacesByCompanyIdHandler implements IQueryHandler {
  ...
  async execute(
    query: GetWorkspacesByCompanyIdQuery,
  ): Promise<Array<WorkspaceDTO>> {
    return this.workspaceFinder.findByCompanyId(
      WorkspaceCompanyId.fromString(query.id),
    );
  }
}
