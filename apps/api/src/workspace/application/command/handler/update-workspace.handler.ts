...
export class UpdateWorkspaceHandler implements ICommandHandler<UpdateWorkspaceCommand> {
    ...
    async execute(command: UpdateWorkspaceCommand): Promise<Result<null, WorkspaceError>> {

        const id = WorkspaceId.fromString(command.id)
        const workspace = await this.workspaceRepository.find(id)

        if (workspace === null) return new Err(WorkspaceAlreadyExistsError.withId(id));

        workspace.updateName(WorkspaceName.fromString(command.name))
        workspace.updateDescription(WorkspaceDescription.fromString(command.description))
