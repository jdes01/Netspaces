...
describe('DeleteWorkspaceHandler', () => {
    ...
    beforeEach(() => {
        ...
        companyId = WorkspaceCompanyId.fromString("6cfa5a4f-c88d-431c-a925-b25a844f25db");
        ...
        services = []
        existingWorkspace = Workspace.add(id, companyId, name, description, location, services)
        command = new DeleteWorkspaceCommand(id.value);
    });

    it('should delete an existing workspace successfully', async () => {

        const workspaceRepository = new InMemoryWorkspaceRepository([existingWorkspace]);
