...
export class WorkspaceWasDeletedProcessManager implements IEventHandler<WorkspaceWasDeletedEvent> {
    constructor(
        @Inject(SPACE_FINDER)
    ...
    async handle(event: WorkspaceWasDeletedEvent) {
        const workspaceSpaces = await this.spaceFinder.findByWorkspaceId(WorkspaceId.fromString(event.id))

        if (workspaceSpaces.length > 0) {
            workspaceSpaces.map(
                (space) => { this.commandBus.execute<ICommand, Result<null, UserError>>(new DeleteSpaceCommand(space._id)); }
            )
        }
    }
}
