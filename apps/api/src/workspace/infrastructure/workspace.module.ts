...
import {
  WorkspaceNameWasUpdatedEvent,
  WorkspaceDescriptionWasUpdatedEvent,
  WorkspaceLocationWasUpdatedEvent,
  WorkspaceWasDeletedEvent,
} from '../domain/event';
import { ProcessManagers } from './process-manager';
import {
  SPACE_PROJECTION,
  SpaceSchema,
} from '../../space/infrastructure/projection';
...
@Module({
  ...
  imports: [
    ...
    EventStoreModule.forFeature([Workspace], {
      ...
      WorkspaceDescriptionWasUpdatedEvent: (
        event: Event<UpdateWorkspaceDescriptionDTO>,
      ) =>
      ...
      WorkspaceLocationWasUpdatedEvent: (
        event: Event<UpdateWorkspaceLocationDTO>,
      ) =>
      ...
      WorkspaceWasDeletedEvent: (event: Event<DeleteWorkspaceDTO>) =>
        new WorkspaceWasDeletedEvent(event.payload._id),
    }),
