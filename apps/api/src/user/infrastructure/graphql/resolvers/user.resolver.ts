...
import {
  ...
  User,
  UserInput,
} from '../schema/user.graphql-model';
...
export class UserResolver {
  ...
  async createUserWithCompany(
    ...
    this.logger.info('Creating user with company', {
      userId: userInput._id,
      companyId: userInput.companyId,
    });

    const createdUserResult = await this.userService.createUserWithCompany(
      userInput._id,
      userInput.name,
      userInput.companyId,
    );
  ...
  async deleteUser(
    ...
    const deletedUserResult = await this.userService.deleteUser(
      deleteUserInput._id,
    );
